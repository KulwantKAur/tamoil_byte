    ...
    var __webpack_modules__ = ({
        ...
        "./resources/js/app.js":
            ...
            checkWeight = function checkWeight() {
                ...
                printGiftCard = function printGiftCard(ordernumber, ean) {
                    showLoader();
                    let value = ean; //Get EAN numbers
                    console.log('scanned value: ' + value.toString());
                    var eans = document.querySelectorAll("tr[data-EAN]");
                    //console.log(eans);
                    var eansArr = [];
                    var eanLength = 0;
                    eans.forEach(function (item) {
                        ...
                        if (!isActiveClassExist && eanLength < item.getAttribute('data-EAN').length) {
                            ...
                            eanLength = item.getAttribute('data-EAN').length;
                            //console.log("ean length: " + eanLength + " value length: " + value.toString().length);
                        }
                    });
                    
                   // console.log('eansArray: ' + eansArr);
                    var isFind = eansArr.indexOf(value.toString());
                    //console.log('Index im Array, wo die gescannte EAN zu finden ist (zeile der Tabelle): ' + isFind + "value of scanned EAN " + value.toString().length);
                    ...
                    if (value.toString().length >= 12 || value.toString().length <= 15) {
                        var itemIndex = scannedProducts.findIndex(function (item) {

                            return item.EAN === value.toString();
                        });
                            ...
                            if (scannedProducts[itemIndex].scanned_quantity <= scannedProducts[itemIndex].quantity) {
                                ...
                                if (scannedProducts[itemIndex].scanned_quantity < scannedProducts[itemIndex].quantity) {
                                    ...
                                    if (item.quantity === item.scanned_quantity) {
                                        ...
                                        sendValue = ordernumber + "x" + ean;
                                       
                                        sendAjax(`/giftcardprint/${sendValue}`, "GET", [], (data) => {
                                            var result = JSON.parse(data.responseText);
                                            console.log(result.giftcard);
                                            if (result.giftcard == 'ok') {
                                                console.log('I made it!');
                                                console.log(result.mulitline);

                                                printJS({
                                                    printable: result.giftcardDocument,
                                                    type: 'pdf',
                                                    base64: true,
                                                    onPrintDialogClose: function onPrintDialogClose() {
                                                        setTimeout(function () {
                                                            ...
                                                            if (result.mulitline) {
                                                                hideLoader();
                                                                console.log('more lines to fulfill');
                                                                //neue Funktion -> close order in Controller
                                                                // sendAjax(`/giftcardprint/close/${ordernumber}`, "GET", [], (data) => {
                                                            ...
                                                            //     location.href = "/qualitynew";
                                                                ...
                                                                sendAjax(`/giftcardprint/close/${ordernumber}`, "GET", [], (data) => {
                                                                    ...
                                                                    if (result.close) {
                                                                        ...
                                                                        showMessageBox("upps... da ist was schief gegangen.");
                                                                    }
...
console.log('barcode change');
                ...
                getProductByEanJnj = function getProductByEanJnj(e) {
                    var value = e.target.value;
                  ...
                  <td><select id=\"reason_code_" + product.ean + "\">" + reason_code + "</select></td>\n\
                  <td><input name=\"product_grade_" + product.ean + "\" type=\"radio\" value=\"A-Ware\" checked>A-Ware<input name=\"product_grade_" + product.ean + "\" type=\"radio\" value=\"B-Ware\">B-Ware<input name=\"product_grade_" + product.ean + "\" type=\"radio\" value=\"Defekt\">Defekt</td>\n\
                  <td><input id=\"bemerkung_ "+ product.ean + "\" type=\"text\" ></td>\n\
                  </tr>"); //   var producstTable = document.getElementById("products");
                                        //   producstTable.insertAdjacentHTML("beforeend", trContent);
                                        //   var focusedElements = document.getElementsByClassName("focused");
                ...
                updateProductsStockCodesJnj = function updateProductsStockCodesJnj() {
                    var quantities = document.getElementsByClassName("delta-quantity");
                    showLoader();
                    if (quantities.length) {
                        var data = {
                        ...
                        $(".pro_ean").each(function () {
                            ...
                            var aa = {
                                ...
                                'ean': this.innerText,
                                'reason_code': reason_code,
                                'bemerkung': $("bemerkung_" + this.innerText),
                                'delta_quantity': {
                                    "stockid": null,
                                    "quantity": $(".delta_" + this.innerText).val(),
                                    "stock_current": null
